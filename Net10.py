...
class AttentionModule(nn.Module):
    def __init__(self, dim):
        ...
        self.conv3 = nn.Conv2d(dim, dim, 3, stride=1, padding=1)
        self.conv1 = nn.Conv2d(dim, dim, 1)

    def forward(self, x):
        u = x.clone()
        attn = self.conv3(x)
        attn = self.conv0(attn)+attn
...
class SLKA(nn.Module):
    def __init__(self, n_feats, k=21, d=3, shrink=0.5, scale=2):
        ...
        self.conv5 = nn.Conv2d(f, f, kernel_size=5, stride=1, padding=2, groups=f)
        self.conv3h = nn.Conv2d(f, f, kernel_size=(1, 3), stride=(1, 1), padding=(0, (3 - 1) // 2), groups=f)
        self.conv3v = nn.Conv2d(f, f, kernel_size=(3, 1), stride=(1, 1), padding=((3 - 1) // 2, 0), groups=f)
        self.conv5h = nn.Conv2d(f//2, f//2, kernel_size=(1, 5), stride=(1, 1), padding=(0, (5 - 1) // 2), groups=f//2)
        self.conv5v = nn.Conv2d(f//2, f//2, kernel_size=(5, 1), stride=(1, 1), padding=((5 - 1) // 2, 0), groups=f//2)

        self.conv1h = nn.Conv2d(f // 2, f // 2, 1, 1)
        self.conv1v = nn.Conv2d(f // 2, f // 2, 1, 1)

        self.conv_spatial_h = nn.Conv2d(f//2, f//2, kernel_size=(1, 7), stride=(1, 1), padding=(0, 9), groups=f//2, dilation=3)
        self.conv_spatial_v = nn.Conv2d(f//2, f//2, kernel_size=(7, 1), stride=(1, 1), padding=(9, 0), groups=f//2, dilation=3)
    ...
    def forward(self, x):
        ...
        attn1 = self.conv3v(self.conv3h(c1))

        at_1, at_2 = torch.chunk(attn1, 2, dim=1)

        attn1_1 = self.conv1h(at_1)
        attn1_2 = self.res_2(attn1_1)
        attn = self.conv5h(attn1_2)
        attn2 = self.activation(attn)
        c3 = self.conv5v(attn2)

        attn2_1 = self.conv1v(at_2)
        attn2_2 = self.res_1(attn2_1)
        c5 = self.conv_spatial_h(attn2_2)
        c5 = self.activation(c5)
        c5 = self.conv_spatial_v(c5)

        c6 = self.conv5(self.ca(attn1))
